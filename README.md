...
#### On Linux
Create a shared Docker network first, then run MySQL on it:

```bash
docker network create stocks-net
...
#### On Linux
Run the app on the same `stocks-net` network, using the MySQL container name as the host:
